...
for (const game of games) {
    ...
    const minCubesNecessary = {red: 0, green: 0, blue: 0};

    for (const draw of draws) {
        const colors = draw.split(',');

        for (const oneColorOfCubes of colors) {
            let [count, color] = oneColorOfCubes.trim().split(' ');

            count = Number(count)

            if (minCubesNecessary[color] < count) {
                minCubesNecessary[color] = count
            }

        }
    }
    const power = minCubesNecessary.red * minCubesNecessary.green * minCubesNecessary.blue
    sumOfPowers += power;
}
