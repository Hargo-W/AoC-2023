...
for (const game of games) {
    ...
    for (const draw of draws) {
        ...
        const gameInfo = {red: 0, green: 0, blue: 0};

        for (const oneColorOfCubes of colors) {
            const [count, color] = oneColorOfCubes.trim().split(' ');
            gameInfo[color] += parseInt(count);
        }
        if (gameInfo.red > 12 || gameInfo.green > 13 || gameInfo.blue > 14) {
            gameValid = false;
        }
    }

    if (gameValid) {
        sumOfIds += parseInt(gameName.split(' ')[1])
    }
}
