...
const initializeArrays = (lines) => {
    const charArrays = [];

    const lineLength = lines[0].length + 2;
    const dotLine = Array(lineLength).fill('.');
    charArrays.push(dotLine);

    for (const line of lines) {
        const charArray = ['.', ...line.split(''), '.'];
        charArrays.push(charArray);
    }
...
const charArrays = initializeArrays(lines);
...
const adjacentNumbers = (line, char) => {
    const directions = [];

    const topLeft = isNumber(charArrays[line - 1][char - 1]);
    const topCenter = isNumber(charArrays[line - 1][char]);
    const topRight = isNumber(charArrays[line - 1][char + 1]);
    const left = isNumber(charArrays[line][char - 1]);
    const right = isNumber(charArrays[line][char + 1]);
    const bottomLeft = isNumber(charArrays[line + 1][char - 1]);
...
const getNumberFromArray = (line, char) => {
    let number = '';
    if (isNumber(charArrays[line][char - 1])) {
        if (isNumber(charArrays[line][char - 2])) {
            number += charArrays[line][char - 2]
        }
        number += charArrays[line][char - 1]
    }
    number += charArrays[line][char]
    if (isNumber(charArrays[line][char + 1])) {
        number += charArrays[line][char + 1]
...
for (const cArray of charArrays) {
    for (let i = 0; i < cArray.length; i++) {
        if (isAsterisk(cArray[i])) {
            const directions = adjacentNumbers(charArrays.indexOf(cArray), i)
            if (directions.length === 2) {
                const firstNumber = getNumberFromArray(directions[0][0], directions[0][1])
                const secondNumber = getNumberFromArray(directions[1][0], directions[1][1])
                const gearRatio = firstNumber * secondNumber
                console.log(charArrays.indexOf(cArray), i, firstNumber, secondNumber)
