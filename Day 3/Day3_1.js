...
const checkForAdjacentSpecialCharacter = (line, char) => {
    let left;
    let right;
    let topLeft;
    let topCenter;
    ...
    if (char > 0) {
        left = isSpecialCharacter(charArrays[line][char - 1])
        if (line > 0) {
            topLeft = isSpecialCharacter(charArrays[line - 1][char - 1])
        }
        if (line < charArrays.length - 1) {
            bottomLeft = isSpecialCharacter(charArrays[line + 1][char - 1])
        }
    }
    if (char < charArrays[line].length  - 1) {
        right = isSpecialCharacter(charArrays[line][char + 1])
        if (line > 0) {
            topRight = isSpecialCharacter(charArrays[line - 1][char + 1])
        }
        if (line < charArrays.length - 1) {
            bottomRight = isSpecialCharacter(charArrays[line + 1][char + 1])
        }
    }
    if (line > 0) {
        topCenter = isSpecialCharacter(charArrays[line - 1][char])
    }
    if (line < charArrays.length - 1) {
        bottomCenter = isSpecialCharacter(charArrays[line + 1][char])
    }

    return left || right || topLeft || topCenter || topRight || bottomLeft || bottomCenter || bottomRight
}

for (const cArray of charArrays) {
    for (let i = 0; i < cArray.length; i++) {
        if (!isNaN(parseInt(cArray[i]))) {
            let isPartNumber = false;
            const firstDigit = cArray[i];
            let secondDigit = null;
            let thirdDigit = null;

            if (checkForAdjacentSpecialCharacter(charArrays.indexOf(cArray), i)) {
                isPartNumber = true
            }

            if (!isNaN(parseInt(cArray[i + 1]))) {
                i++;
                secondDigit = cArray[i]
                if (!isPartNumber && checkForAdjacentSpecialCharacter(charArrays.indexOf(cArray), i)) {
                    isPartNumber = true
                }
            ...
            if (isPartNumber) {
                const number = (firstDigit ?? '') + (secondDigit ?? '') + (thirdDigit ?? '');
