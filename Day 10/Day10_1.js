const tileField = `L777FJF77F-77-F-7F7.FF.J7.-J--J7F|7F.FJ.L-|F|-F|FJ7-J7F-JFF-7.L7FL7J-FF|7--7.L--|77.F-J77.-7FJ7F7FF--.FFJ7-7-F|.FL-L-F7.F|-FF7F|-7-|7-.FF-7.
...
L-JJL-JJJL7JL-L-JJJFJJLF-L-.F7.|LLL--J7J-LFL7L|JJJF-J7..L-JLJJL---J-LF-LJJL7.LLJJJJJLJ.L|J-LFLJL-L--|JJ-LLJL|J-|.-J|.L|JL|JJ.FF--7L-FJ-LJ.J-`.split('\n').map(row => row.split(''));

tileField.unshift(Array(tileField[0].length).fill('.'));

tileField.push(Array(tileField[0].length).fill('.'));

for (let i = 0; i < tileField.length; i++) {
    tileField[i].unshift('.');
    tileField[i].push('.');
}
...
for (let row = 0; row < tileField.length; row++) {
    for (let col = 0; col < tileField[row].length; col++) {
        if (tileField[row][col] === 'S') {
            // +1 is to set the starting location based on an eligible tile next to S
...
let stepCount = 1;
let prevDirection = 'north';

const moveStep = () => {
    const standingOn = tileField[currentLocation[0]][currentLocation[1]]

    stepCount++
    if (prevDirection !== 'south' && ( standingOn === '|' || standingOn === 'J' || standingOn === 'L' )) {
        currentLocation = [currentLocation[0] - 1, currentLocation[1]]
        prevDirection = 'north'
    } else if (prevDirection !== 'west' && ( standingOn === '-' || standingOn === 'L' || standingOn === 'F' )) {
        currentLocation = [currentLocation[0], currentLocation[1] + 1]
        prevDirection = 'east'
    } else if (prevDirection !== 'north' && ( standingOn === '|' || standingOn === '7' || standingOn === 'F' )) {
        currentLocation = [currentLocation[0] + 1, currentLocation[1]]
        prevDirection = 'south'
    } else if (prevDirection !== 'east' && ( standingOn === '-' || standingOn === '7' || standingOn === 'J' )) {
        currentLocation = [currentLocation[0], currentLocation[1] - 1]
        prevDirection = 'west'
    }
}

while (tileField[currentLocation[0]][currentLocation[1]] !== 'S') {
    moveStep()
}
