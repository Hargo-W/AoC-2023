...
const valuesToSearchFor = ['1','2','3','4','5','6','7','8','9','one','two','three','four','five','six','seven','eight','nine']

const getCalibrationValueFromStart = (line) => {
    let calibrationValue = null;
    let indexOfLastValue = 9999;

    for (let y = 0; y < valuesToSearchFor.length; y++) {
        const indexOfCurrentValue = line.indexOf(valuesToSearchFor[y]);
        if (indexOfLastValue > indexOfCurrentValue && indexOfCurrentValue != -1) {
            calibrationValue = getString(valuesToSearchFor[y]);
            indexOfLastValue = indexOfCurrentValue;
        }
    }
    return calibrationValue;
}

const getCalibrationValueFromEnd = (line) => {
    let calibrationValue = null;
    let indexOfLastValue = -1;

    for (let z = 0; z < valuesToSearchFor.length; z++) {
        const indexOfCurrentValue = line.lastIndexOf(valuesToSearchFor[z].toString());

        if (indexOfLastValue < indexOfCurrentValue) {
            calibrationValue = getString(valuesToSearchFor[z]);
            indexOfLastValue = indexOfCurrentValue;
        }
    }
    return calibrationValue;
...
for (let x = 0; x < separateLines.length; x++) {
    const cStart = getCalibrationValueFromStart(separateLines[x]);
    const cEnd = getCalibrationValueFromEnd(separateLines[x]);
    const cValue = cStart + cEnd
    totalSum = totalSum + Number(cValue);
}
